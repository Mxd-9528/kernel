...
def compress(messages, model, call):
    """把待压消息拼成压缩请求并调用模型，返回摘要文本。call(msgs, model) 由调用方传入（compact 不依赖 call）。

    这是 compact 的默认 call_compress 实现，提出来才能单测 COMPRESS_PROMPT 拼装。
    """
    import json
    req = [
