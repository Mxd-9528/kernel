...
def _mcp_call(proc: subprocess.Popen, req: dict) -> dict | None:
    """向 MCP 进程发一个 JSON-RPC 请求，等回对应 id 的响应（跳过中间通知）。"""
    assert proc.stdin is not None and proc.stdout is not None  # 调用方以 PIPE 起进程
...
def mcp(server: str, tool: str | None = None, **params):
    ...
    try:
        ...
        assert proc.stdin is not None  # 起进程时以 PIPE 打开
        proc.stdin.write(json.dumps({"jsonrpc": "2.0", "method": "notifications/initialized"}) + "\n")
        proc.stdin.flush()

        if tool is None:
            resp = _mcp_call(proc, {"jsonrpc": "2.0", "id": 2, "method": "tools/list", "params": {}})
            assert resp is not None  # MCP 握手已完成，list 必有响应
