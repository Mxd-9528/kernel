...
| 模块 | 职责 | 被谁依赖 |
|------|------|----------|
| `main.py` | 入口：显式注册事件处理器，启动 chat | — |
| `chat.py` | 对话主循环：读输入、处理斜杠命令、调用 agent | main |
| `agent.py` | 决策-执行-观察循环 + 事件钩子（on/emit） | chat, compact, history, display |
...
| `history.py` | 对话历史持久化到 history.json | chat, agent |
| `compact.py` | 上下文压缩：保留 system + 最近 6 轮，摘要中间 | agent |
| `display.py` | 流式输出渲染 | agent |
...
agent.py 的事件钩子是唯一的扩展点。模块导出回调函数，main.py 通过 `on(EVENT_X)(handler)` 显式注册。agent 循环在关键节点 `emit()`：

- `before_send`：发 LLM 请求前 → compact 压缩上下文
- `thinking_delta`：收到 reasoning token → display 更新思考中 spinner
- `display_delta`：收到正文 token → display 更新回复中 spinner
- `display_flush`：流结束 → display 一次性渲染累积正文
- `save`：消息列表变更 → history 存盘

**远期参考**：当 agent() 需要独立单元测试（不加载消费者模块），或需以不同观察者组合运行多个实例时，将 `_hooks` 全局注册表替换为 Protocol + CompositeObserver——Observer 模式的经典实现，让 agent() 接收 observer 参数而非依赖模块全局状态。
