"""在持久内核里执行代码块，返回三元组 Result。

对外契约（消费者读这段即可，不必下潜）：

    run(code) -> Result
      正常: Body=表达式值 repr / 或 stdout；.error=None
      异常: Body="";        .error=原生异常对象；facts.stdout=traceback；[facts.stderr]
      超时(>60s 转后台): Body=引导文本；.error=TimeoutError；
                        facts.task_id=<句柄>；facts.status="background"
                        —— 用 task_status(id) / task_cancel(id) 后续操作
